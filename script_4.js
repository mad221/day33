...
console.log("il y a "+ " " + array.length + "journaliste");


let count = 0;
array.forEach(journaliste => {
  if (hasNumber(journaliste)) {
    count++;
  }
});

console.log(`Nombre d'handles contenant un numero : ${count}`);
count=0;

array.forEach(word => {
  if (word.includes("aude") || word.includes("Aude"))
  {
    count++;
  }});
  console.log(`Nombre d'aude : ${count}`);
  ...
  array.forEach(word => {
    ...
    console.log(`Nombre de maj a la premiere lettre : ${count}`);
...
console.log("le nombre de journaliste ayant une maj est de " + total);
...
console.log("le nombre de journaliste ayant un _ est de " + count);


console.log("la position de epenser est " +array.indexOf("@epenser"));


array.sort(function(a, b){return a.length-b.length});
console.log(array.slice(0,50));
